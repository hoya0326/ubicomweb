...
function getVoters(poll, optId) {
    ...
    return poll.votes
        ...
        .map(v => v.userName || v.userId || "알 수 없음");
}
...
async function saveVote(poll, optionIds) {
    try {
        ...
        const res = await fetch(`${API_BASE_URL}/polls/${poll.id}/vote`, {
            method: "POST",
...
async function deletePollFromStorage(poll) {
    try {
        const res = await fetch(`${API_BASE_URL}/polls/${poll.id}`, {
            method: "DELETE"
        });
        if (!res.ok) {
            alert("투표 삭제에 실패했습니다.");
...
function renderResultBar(poll) {
    ...
    return poll.options.map(opt => {
        ...
        const isMyChoice = myVote && myVote.optionIds && myVote.optionIds.some(id => String(id) === String(opt.id));
...
async function submitVote(pollId) {
    ...
    const success = await saveVote(poll, sel);
    if (success) {
...
async function execDelete(pollId) {
    const poll = polls.find(p => matchId(p.id, pollId));
    if (!poll) return;
    const success = await deletePollFromStorage(poll);
    if (success) {
        await loadPolls();
        renderAll();
    }
...
async function closePollByAdmin(pollId) {
    if (!currentUser || !currentUser.isAdmin) {
        alert("관리자 권한이 필요합니다.");
        return;
    }
    if (confirm("이 투표를 마감 처리하시겠습니까?")) {
        ...
        const success = await closePollInStorage(poll);
        if (success) {
            await loadPolls();
            renderAll();
        }
...
async function submitCreatePoll() {
    ...
    try {
        ...
        if (!res.ok) {
            ...
            showFormError(errData.message || "투표 생성에 실패했습니다.");
            return;
        }
